...
import { Users, FileQuestion, Trophy, Plus, Trash2, LogOut, Settings, ClipboardList, RotateCcw, RefreshCw, Library } from 'lucide-react';
...
export function AdminDashboard({
  ...
  const currentRoundQuestions = questions.filter(q => q.round === currentRound && q.isActive);
        ...
        <Tabs defaultValue="teams" className="space-y-4">
          ...
          <TabsContent value="question-bank" className="space-y-4">
            <QuestionBank
              ...
              onActivateQuestions={onActivateQuestions}
            />
          </TabsContent>
          ...
          <TabsContent value="questions" className="space-y-4">
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle>Active Questions in Spin Wheel - Round {currentRound}</CardTitle>
                    <CardDescription>
                      Questions that are currently available for teams to spin
                    </CardDescription>
                  </div>
                </div>
              </CardHeader>
                ...
                {currentRoundQuestions.length === 0 && (
                  <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg dark:bg-blue-950/30 dark:border-blue-800">
                    <p className="text-blue-800 text-sm dark:text-blue-200">
                      💡 <strong>Tip:</strong> No active questions yet. Go to the <strong>Question Bank</strong> tab to add questions and activate them for the spin wheel.
                    </p>
                  </div>
                )}
                <Table>
