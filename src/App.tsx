...
export default function App() {
  ...
  const handleAddQuestion = (question: string, description: string, round: number) => {
    const newQuestion: Question = {
      ...
      description,
      isLocked: false,
      isActive: false, // Add to question bank, not active yet
    };
  ...
  const handleActivateQuestions = (questionIds: string[]) => {
    const updatedQuestions = questions.map(q => 
      questionIds.includes(q.id) ? { ...q, isActive: true } : q
    );
    setQuestions(updatedQuestions);
    saveQuestions(updatedQuestions);
  };
  ...
  const handleResetCurrentRound = () => {
    ...
    // Unlock all active questions in current round (keep isActive as is)
    const updatedQuestions = questions.map(q => {
      if (q.round === currentRound && q.isActive) {
  ...
  const handleResetAllRounds = () => {
    ...
    // Unlock all questions and deactivate them (move back to bank)
    const updatedQuestions = questions.map(q => ({
      ...q,
      isLocked: false,
      assignedToTeamId: undefined,
      isActive: false, // Move all questions back to bank
    }));
    setQuestions(updatedQuestions);
    saveQuestions(updatedQuestions);
      ...
      {!user.role ? (
        ...
        <>
          <AdminDashboard
            ...
            onActivateQuestions={handleActivateQuestions}
